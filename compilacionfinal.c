...
int main() {
  ...
  for(int sentido=0;sentido<2;sentido++)
  {
    for(N_betas=1;N_betas<N_pasos;N_betas++)
    {
      ...
      ecv=cs_error(10,10,eres); /// LOS DOS PRIMEROS VALORES DE AQUÍ LOS HE PUESTO POR PONER
...
{                                                                        //n_bloques*n_datblo <= N_m (idealmente igual), vnorm: vector de e o m para un beta
    ...
    double sumb=0,sumb2=0, gb[n_bloques], sumg=0, sumg2=0;               //Con la desviación típica de estos CV o X estima el error, pero el valor real se calcula sin
    for(ics=0;ics<n_bloques;ics++)                                       //análisis por bloques.
    {
...
{                                                                        //Entradas: las mismas que el anterior. Calcula las medias de los bloques y estima el error
    int iem, jem;                                                        //con la desviación típica entre las medias (análisis por bloques). Tendremos que ver
